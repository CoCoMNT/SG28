...
class AopClient {
	...
	protected function sign($data, $signType = "RSA") {
		...
		if ("RSA2" == $signType) {
            $rt = openssl_sign($data, $sign, $res, OPENSSL_ALGO_SHA256);
            if (!$rt) {
                exit("验证失败：检查公钥和私钥是否填写正确");
            }
		} else {
		...
		$sign = base64_encode((string)$sign);
		return $sign;
	}
	...
	*/
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		...
		if ("RSA2" == $signType) {
			$result = (bool)openssl_verify($data, base64_decode((string)$sign), $res, OPENSSL_ALGO_SHA256);
		} else {
			$result = (bool)openssl_verify($data, base64_decode((string)$sign), $res);
		}
